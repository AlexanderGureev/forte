...
export function getStudioCameraPlacement(
  ...
  const halfHorizontalFov = Math.atan(Math.tan(halfVerticalFov) * Math.max(aspect, 0.2));
  const requiredHalfWidth = (keyboardWidth * getWidthMargin(viewport, viewportWidthPx, hudSidePx)) / 2;
  const distance = Math.max(requiredHalfWidth / Math.tan(halfHorizontalFov), 16);
